...
export default function Dashboard() {
  ...
  const fetchCommentCounts = (rows: BankRecord[]) => {
    Promise.all(
      rows.map((r) =>
  ...
  useEffect(() => {
    fetchWithAuth(`${BASE}/api/user-service/write/get-all-countries-bank-data`)
      ...
      .then((rows: BankRecord[]) => { setData(rows); fetchCommentCounts(rows); })
      .catch((e) => setError(e.message))
      .finally(() => setLoading(false));
  }, []);

  const refreshCount = (bankId: string) => {
